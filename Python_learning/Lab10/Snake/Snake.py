...
def showLast(user):
    com = """
        SELECT * FROM snakeStats WHERE username = '{}'
        ORDER BY id DESC;
    """.format(username)
    conn = None
    try:
...
insertCom = """
            INSERT INTO snakeStats(username, level, score)
            VALUES('{}', '{}', '{}') RETURNING username, level, score;
        """
...
class Food:
    ...
    def eaten(self, snake):
        ...
        while self.location in snake:
            self.location = Point(random.randint(0, 29), random.randint(0, 24))
        score += random.randint(20, 50)  # different weight of food
    # Function for disappearance
    def disap(self, snake):
        self.location = Point(random.randint(0, 29), random.randint(0, 24))
        while self.location in snake:
            self.location = Point(random.randint(0, 29), random.randint(0, 24))
...
def main():
    ...
    pygame.time.set_timer(disapear, 10000)
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == disapear:
                food.disap(snake.body) # event will be activated after 10000 milis, and food will be teleported
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_s:
                    do(insertCom.format(username, l, score))
                    Pause = not Pause
        if not Pause:
            snake.move()
...
main()
do(insertCom.format(username, level, score))
